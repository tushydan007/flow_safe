...
export const fetchAlerts = createAsyncThunk(
  'alerts/fetchAlerts',
  async (params: { acknowledged?: boolean; severity?: string } = {}, { rejectWithValue }) => {
    try {
...
export const acknowledgeMultipleAlerts = createAsyncThunk(
  ...
  async (alertIds: number[], { rejectWithValue }) => {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to acknowledge alerts';
      return rejectWithValue(message);
    }
  }
);
  ...
  reducers: {
    addNewAlert: (state, action: PayloadAction<Alert>) => {
      ...
      if (!action.payload.is_acknowledged) {
        ...
        state.severityCounts[severity] = (state.severityCounts[severity] || 0) + 1;
      }
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(acknowledgeAlert.fulfilled, (state, action) => {
        ...
        state.criticalAlerts = state.criticalAlerts.filter(a => a.id !== alertId);
