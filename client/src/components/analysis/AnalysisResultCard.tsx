...
export function AnalysisResultCard({
  ...
  const info = analysisTypeInfo[result.analysis_type as keyof typeof analysisTypeInfo];
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold">{info?.title || result.analysis_type_display}</h3>
      ...
      {result.status === 'processing' && (
        <div className="mb-3">
          ...
          <p className="text-xs text-muted-foreground mt-1">{result.progress}% complete</p>
        </div>
      )}
          ...
          {result.severity && (
            <span
              className={cn(
                ...
                result.severity === 'medium' && 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400',
                result.severity === 'low' && 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'
              )}
            >
              {result.severity}
            </span>
